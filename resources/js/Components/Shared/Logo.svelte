<script>
    ...
    const sizes = {
        ...
        default: { box: 'w-10 h-10', icon: 'text-base', text: 'text-lg', padding: 'p-2' },
...
<Link {href} class="{layout === 'vertical' ? 'inline-flex flex-col items-center' : 'flex items-center'} group">
    <div class="rounded-xl flex items-center justify-center shadow-lg transform group-hover:scale-110 transition-transform duration-200 {currentSize.box} {currentSize.padding}"
         style="background: linear-gradient(to bottom right, var(--theme-primary-500), var(--theme-primary-700))">
        <svg class="fill-current text-white {currentSize.icon} w-full h-full" viewBox="0 0 50 52" xmlns="http://www.w3.org/2000/svg">
    ...
    {#if showText}
    <div class="{layout === 'vertical' ? 'mt-3 text-center' : 'ml-3'} transition-all duration-300">
        <span class="font-bold {textColor} {currentSize.text}" style="text-shadow: 0 1px 2px rgba(0,0,0,0.05);">
            Starkit Svelte
        </span>
        {#if showTagline}
